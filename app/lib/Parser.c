...
STATE_MACHINE_RETURN_VALUE parse_char_(uint8_t current_char, uint16_t *state, bool *no_plus_flag, bool *multiple_plus_flag) {
    switch (*state) {
        ...
        case 13: {  // "+[chars]<CR><LF>"
            ...
            if (current_char == '+') {
                *multiple_plus_flag = true;
                *state = 10;
                break;
            }
            if (*multiple_plus_flag) {
        ...
        case 19: {  //+[chars]<CR><LF>[chars]<CR><LF>
            if (current_char == '\r') {
                *state = 20;
                break;
            }
        ...
        case 20: {  //+[chars]<CR><LF>[chars]<CR><LF><CR>
            if (current_char == '\n') {
                file_new_line();
                *state = 21;
                break;
        ...
        case 21: {  //+[chars]<CR><LF>[chars]<CR><LF><CR><LF>
            if (current_char == 'O') {
                *state = 3;
                break;
            } else if (current_char == 'E') {
...
STATE_MACHINE_RETURN_VALUE parse_char(uint8_t current_char) {
    ...
    static bool no_plus_flag = false;
    static bool multiple_plus_flag = false;

    STATE_MACHINE_RETURN_VALUE ret = parse_char_(current_char, &state, &no_plus_flag, &multiple_plus_flag);
